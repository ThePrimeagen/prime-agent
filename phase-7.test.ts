...
test("parsePhaseNumber accepts 1-7", () => {
  ...
  expect(parsePhaseNumber("6")).toBe(6);
  expect(parsePhaseNumber("7")).toBe(7);
  expect(parsePhaseNumber(" 3 ")).toBe(3);
});

test("parsePhaseNumber rejects empty, non-numeric, and out-of-range", () => {
...
test("assertResumeMode allows review for 2/3/4/6/7", () => {
  for (const phase of [2, 3, 4, 6, 7] as const) {
...
test("pipelineStepsFrom starts at phase 6", () => {
  expect(pipelineStepsFrom(6, "phase")).toEqual([
    { kind: "phase", phase: 6 },
    { kind: "review", phase: 6 },
    { kind: "phase", phase: 7 },
    { kind: "review", phase: 7 },
...
test("pipelineStepsFrom starts at phase 6 review", () => {
  expect(pipelineStepsFrom(6, "review")).toEqual([
    { kind: "review", phase: 6 },
    { kind: "phase", phase: 7 },
    { kind: "review", phase: 7 },
  ]);
