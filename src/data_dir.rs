...
/// Resolve data directory: CLI `--data-dir` wins, then merged `data-dir` from config, else cwd.
pub fn resolve_data_dir(cli_data_dir: Option<&Path>, config_data_dir: Option<&Path>) -> Result<PathBuf> {
    if let Some(p) = cli_data_dir {
        return Ok(expand_path(p));
    }
    if let Some(p) = config_data_dir {
