/**
 * Per-worker `prime-agent serve` with live reload enabled (fs watcher + WS broadcasts).
 */
...
const PORT_BASE = 19080;
const DATA_GLOB = ".tmp/e2e_live_data_w";
...
>({
  _primeWorkerServer: [
    async ({}, use, workerInfo) => {
      ...
      delete env.PRIME_AGENT_DISABLE_LIVE_RELOAD;
      const child = spawn(
        "cargo",
