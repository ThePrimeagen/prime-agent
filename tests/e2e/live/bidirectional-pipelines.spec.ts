...
test("WebSocket create step with empty description round-trips in pipeline.json (live)", async ({
  page,
  e2eDataDir,
}) => {
  const suffix = `${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
  const name = `bi-pipe-empty-${suffix}`;
  const title = `step-empty-${suffix}`;

  await page.goto("/");
  expect((await wsCreatePipeline(page, name)).ok).toBe(true);
  expect((await wsCreateStep(page, name, title, "")).ok).toBe(true);
